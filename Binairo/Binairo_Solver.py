### Taking Input
### Het speelveld word gegeven als lijst met lijsten die dan gevuld moeten worden met een 1, een 0 of een "." Een punt stelt een leeg vakje voor
def fieldGenerator():
    rowAmount = int(input("Hoeveel rijen/kolommen zijn er: "))
    listOfRows = []
    rijnummer = 0
    while rijnummer < rowAmount:
        iRow = []
        for i in range(rowAmount):
            iRow.append(input(f"Geef het teken voor rij {rijnummer + 1} positie {i+1}: "))
        listOfRows.append(iRow)
        rijnummer += 1
